...
- [Examples](#examples)
  ...
  - [Define Single Expectations or Expectation Groups](#define-single-expectations-or-expectation-groups)
...
The statement can be read as "I expect, x to equal nine" where an equality check is used (for an identity check, you have to use `toBeSameInstanceAs`). 
Since this is false, an `AssertionError` is thrown with a corresponding message as shown in the [Output](#ex-first)
where `◆ ...` represents a single expectation for the subject (`10` in the above example) of the expectation.
In this sense the report can be read as `I expected the subject of the expectation, which was 10, to equal 9` 
-- and needless to say, this expectation was not met and thus the thrown error.
...
<details>
<summary>👓 further expectation verbs...</summary>

Atrium provides two further expectation verbs next to `expect` out of the box: `assert` and `assertThat`
...
Yet, you can also [define your own expectation verb](#use-own-expectation-verb) if another is your favourite.
<hr/>
</details> 

The next section shows how you can define multiple expectations for the same subject.

## Define Single Expectations or Expectation Groups
<ex-single>
...
you only need to write the `expect(...)` part once and can make several single expectations for the same subject.
The expression which determines the subject of the expectation (`4 + 6` in the above example) is evaluated only once. 
...
An expectation group throws an `AssertionError` at the end of its block; hence reports that both expectations do not hold.
...
You can use `and` as filling element between single expectations and expectation group blocks:
```kotlin
...
You can also pass a lambda to `expect` and then use `toThrow` to define the expectation that 
invoking the lambda (with `invoke`) throws a certain exception (`IllegalStateException` in the example above).
...
- the second expects an `expectationCreator`-lambda in which you can define sub-expectations. 
  An `expectationCreator`-lambda has always the semantic of an [expectation group block](#define-single-expectations-or-expectation-groups). 
...
<a name="property-extractor"></a>
<a name="method-extractor"></a>
## Feature Extractors
Many times you are only interested in certain features of the subject and want to make expectations about them. 
...
So the above output can be read as "I expected the subject of the expectation, which was actually the Person(...), respectively its property `isStudent`, which was `false`, to equal `true`. 
The second feature is not shown in reporting as the expectations for the first already failed and
we used the [single expectation](#define-single-expectations-or-expectation-groups) syntax which has fail-fast semantic.
...
- the second expects an `expectationCreator`-lambda in addition, in which you can define sub-expectations for the feature.
  An `expectationCreator`-lambda has always the semantic of an [expectation group block](#define-single-expectations-or-expectation-groups) or in other words, not-fail fast.
  It has also the benefit, that Atrium can provide those sub-expectations in error reporting, 
...
<details>
<summary>💬 Provide a feature extractor for every property? </summary>

You might be asking yourself whether it is better to [write an own expectation function](#write-own-expectation-functions) or use `feature`. 

The only drawback of using an existing property is that a few more key strokes are required compared to 
[writing an own expectation function](#write-own-expectation-functions) once and then reuse it (as we did with `message`).
Yet, we do not recommend writing an own expectation function for every single property.
We think it makes sense to add one if you use it a lot and (preferably) it is a stable API. 
Why not always? Because one quickly forgets to rename the expectation function 
if the property as such is renamed (e.g., as part of an IDE refactoring). 
As you can see, you would need to keep the property name and the name of the expectation function in sync to be meaningful 
(otherwise one gets quickly confused or has to remember two names for the same thing). 

Writing expectation functions for methods is a different story though, especially due to [overload bugs in Kotlin](https://github.com/robstoll/atrium/wiki/Kotlin-Bugs-and-missing-features).
...
sense to wrap it into an expectation function and use `_logic.extractFeature` instead.
...
then we suggest that you [write a specific expectation function](#write-own-expectation-functions) for it.
...
Atrium provides a way to extract features in a safe way in case the extraction is only valid for certain subjects.
...
This version of `feature` provides two different kind of overloads as well:
...
Yet, if you use the same feature extractor often or it gets more complicated, 
then it might be worth to [write an own expectation function](#write-own-expectation-functions).

### Within Expectation Functions

In case you write an own expectation function, then we discourage to use the `MetaFeature`-provider-lambda 
...
```
↑ <sub>[Example](https://github.com/robstoll/atrium/tree/master/misc/tools/readme-examples/src/main/kotlin/readme/examples/ReadmeSpec.kt#L170)</sub> ↓ <sub>[Output](#ex-within-expectation-functions)</sub>
...
Also this version of `feature` provides to kind of overloads, one without and one with `expectationCreator`-lambda.
...
expect(x).isA<SubType1>()
    ...
    .toBe(2)
```
↑ <sub>[Example](https://github.com/robstoll/atrium/tree/master/misc/tools/readme-examples/src/main/kotlin/readme/examples/ReadmeSpec.kt#L203)</sub> ↓ <sub>[Output](#ex-type-expectations-1)</sub>
...
```
↑ <sub>[Example](https://github.com/robstoll/atrium/tree/master/misc/tools/readme-examples/src/main/kotlin/readme/examples/ReadmeSpec.kt#L209)</sub> ↓ <sub>[Output](#ex-type-expectations-2)</sub>
...
- the first (shown in the first example) is parameterless and turns only the subject into the expected type; 
  failing to do so cannot include additional information in error reporting though.
- the second (shown in the second example) expects an `expectationCreator`-lambda in which you can define sub-expectations. 
  An `expectationCreator`-lambda has always the semantic of an [expectation group block](#define-single-expectations-or-expectation-groups) 
  -- as a recapitulation, expectations in an expectation group block are all evaluated and failures are reported at the end of the block.
  It has also the benefit, that Atrium can provide those sub-expectations in error reporting, 
  showing some additional context in case of a failure.
...
```text
expected that subject: "postulating expectations made easy"        <1234789>
◆ equals: null
```
</ex-nullable-1>
...
◆ equals: "postulating expectations made easy"        <1234789>
```
</ex-nullable-2>
...
This is a shortcut for `isA<Xy>` where `Xy` is the non-nullable type (see [Type Expectations](#type-expectations)).
Following an example:
...
one without (example above) and one with `expectationCreator`-lambda (example below); see 
...
Atrium provides expectation builders which allow to make sophisticated `contains` expectations for `Iterable<T>`.
...
The expectation function `contains(2, 3)` is a shortcut for using a 
[Sophisticated Expectation Builder](#sophisticated-expectation-builders) -- it actually calls `contains.inAnyOrder.atLeast(1).values(2, 3)`. 
...
The output does not show anything about the expected value `2` because the predefined expectation verbs have configured [`ReporterBuilder`](#reporterbuilder) 
...
which shows us only expectations (or sub expectations) which failed.
...
Atrium allows us to use an `expectationCreator`-lambda to identify an element
(an `expectationCreator`-lambda can also be thought of as matcher / predicate in this context).
An element is considered as identified, if it holds all specified expectations.
Following an example:
...
and a second one which expects one or more `expectationCreator`-lambda, 
...
Notice that you can pass `null` to `containsExactly` instead of an `expectationCreator`-lambda to match `null`.
...
choice if you think in terms of: expect a predicate holds. These two are completed with an `all` expectation function. 
...
To use the expectation builder for sophisticated `Iterable<T>`-contains-expectations, you can type `contains` 
...
- Whether a specified `expectationCreator`-lambda matched (signified by `✔` or `✘`) 
  the corresponding element or not (e.g. `✘ ▶ entry 1:` was `2` and we expected, it `is less than 2`)
- Whether the expected size was correct or not (`✘ ▶ size:` was `4`, we expected it, `to be: 2` -- see also [Property Expectations](#property-expectations))
...
Next to making expectations based on key-value `Pair`s one can also define sub expectations for the value of an entry with 
...
In case you want to only state an expectation about the keys or values of the `Map` then you can use `keys` or `values`:
...
`isKeyValue` as well as `key` and `value` are expectation functions defined for `Map.Entry<K, V>`.
...
Atrium’s expectations for paths give detailed failure hints explaining what happened on the file system.
...
`because` can be a useful tool for explaining an expectation. Sometimes it is not directly obvious why one
...
Having said that, you should not use `because` if you are missing a specific predefined expectation function. You can use
a [feature extractor](#feature-extractors), [write your own expectation function](#write-own-expectation-functions)
...
As an example, Atrium can help you write data driven tests in a common module of a multiplatform-project.

The trick is to wrap your expectations into an [expectation group block](#define-single-expectations-or-expectation-groups)
and create [Feature extractors](#feature-extractors). Following an example:
...
Following another example which involves an `expectationCreator`-lambda and not only a simple `toBe` check. 
We are going to re-use the `myFun` from above:
...
`toBeNullIfNullGivenElse` which accepts an `expectationCreator`-lambda or `null`.
Simplified it is short for `if (expectationCreatorOrNull == null) toEqual(null) else notToBeNull(expectationCreatorOrNull)`. 
...
- [atrium-gradle-testkit](https://github.com/jGleitz/atrium-gradle-testkit): Atrium expectation functions to test Gradle plugins with TestKit.
...
This experience is improved by providing up-to-date [code documentation](#kdoc) (in form of KDoc) for all expectation functions, 
...
Atrium points out which `values` were found, includes an implicit expectation about the size and 
...
The above expectation looks good at first sight but actually fails (at least on my machine). 
...
One last example. This time about the expectation that a certain `Throwable` was thrown where the expectation fails 
because it was the wrong exception type which was thrown. 
...
And if the user should decide to use `isEqualIncludingScale` and at some point an expectation fails only due to the comparison of `BigDecimal.scale`
...
Another example are empty `expectationCreator`-lambdas. 
...
The following sub sections will show how you can write your own expectation functions. 
...
2. *Use the method `createAndAddAssertion`* (provided by `Expect`)  which creates and adds 
    ...
    The method `createAndAddAssertion` returns an `Expect` for the current subject making it easy for you to provide a fluent API as well.
...
Say you want to build an `isBetween` expectation function for `java.util.Date`, you could write it as follows:
...
You need to use an [expectation group block](#define-single-expectations-or-expectation-groups) 
...
Say you want to state an expectation about the number of children a person has:
...
1. we make use of a [feature extractor with class reference](#within-expectation-functions).
2. We use the overload which expects an `expectationCreator`-lambda. This way subsequent expectations are still made on `Person` and not on `children`.
...
We once again use `feature` with an [expectation group block](#define-single-expectations-or-expectation-groups) 
...
(see [feature extractors within expectation functions](#within-expectation-functions)).
...
Enough of feature extractors. Let's move on to an example where we want to reuse an existing function but with different
...
And, it gets worse if we want to use `contains.inAnyOrder.entries` which expects at least one `expectationCreator`-lambda (`Expect<T>.() -> Unit`)
...
Which means, this time we need to use `expectationCreator`-lambdas. This can be written as follows:
...
- `collect`/`collectoForDifferentSubject` which allows collecting expectations - especially helpful in composing expectations (see [DefaultMapLikeContainsAssertions](https://github.com/robstoll/atrium/blob/master/logic/atrium-logic-common/src/main/kotlin/ch/tutteli/atrium/logic/creating/maplike/contains/creators/impl/DefaultMapLikeContainsAssertions.kt#L193))
- `extractFeature` for extracting features which are not always safe to extract (see [`List.get`](https://github.com/robstoll/atrium/tree/master/logic/atrium-logic-common/src/main/kotlin/ch/tutteli/atrium/logic/impl/DefaultListAssertions.kt#L13))   
...
Do you want to write an own sophisticated expectation builder (or extend a current with more options) instead of an expectation function?
Great, we do not provide documentation yet (had only one question about it so far). 
Therefore, please have a look at the implementation, for instance how the sophisticated expectation builders for `Iterable<T>` are defined:
...
Atrium offers three expectation verbs out of the box: `expect`, `assert` and `assertThat`. 

But, you can also define your own set of expectation verbs if they do not suite you or if you do not want that all of them are available in your classpath.
...
What are the benefits of creating own expectation verbs:
- you can limit the set of available expectation verbs. <br/>
...
- you can encapsulate the reporting style. <br/>
  ...
  For instance, you could change from same-line to multi-line reporting or report not only failing but also successful expectations, change the output language etc.
    ...
    We suggest you create an adapter project for Atrium where you specify the expectation verbs. 
...
- you have to maintain your expectation verbs. That should not be a big deal 
...
It does not matter if you use your [own expectation verb](#use-own-expectation-verb) or a predefined one.
...
Typically, you would put `DescriptionIntExpectation` into an own module (jar) 
...
Let us rewrite the `isEven` expectation function from the section [Write own Expectation Functions](#write-own-expectation-functions)
as second example:
...
- [`nullable`](https://github.com/robstoll/atrium/tree/master/logic/atrium-logic-common/src/main/kotlin/ch/tutteli/atrium/logic/utils/nullable.kt#L18)
  turns a type into a nullable type and a return type of a KFunction into a nullable type.
- [`nullableContainer`](https://github.com/robstoll/atrium/tree/master/logic/atrium-logic-common/src/main/kotlin/ch/tutteli/atrium/logic/utils/nullable.kt#L30)
  turns an `Iterable` into an iterable with nullable element type, likewise it does the same for `Array`.
- [`nullableKeyMap`](https://github.com/robstoll/atrium/tree/master/logic/atrium-logic-common/src/main/kotlin/ch/tutteli/atrium/logic/utils/nullable.kt#L56)
  turns a `Map` into a map with a nullable key type.
- [`nullableValueMap`](https://github.com/robstoll/atrium/tree/master/logic/atrium-logic-common/src/main/kotlin/ch/tutteli/atrium/logic/utils/nullable.kt#L69)
  turns a `Map` into a map with a nullable value type.
- [`nullableKeyValueMap`](https://github.com/robstoll/atrium/tree/master/logic/atrium-logic-common/src/main/kotlin/ch/tutteli/atrium/logic/utils/nullable.kt#L82)
...
## Are there contains/any/none/all expectations for `Sequence`/`Array`?

Atrium does not provide extension functions applicable for `Expect<Sequence<E>>` (or `Array`) directly,
...
Likewis,e you can turn an `Expect<Array<E>>`, `Expect<DoubleArray>` etc. into an `Expect<List<E>>` with `asList`.

Feel free vote for [first class support for Array and Sequence](https://github.com/robstoll/atrium/issues/459).
...
## Where do I find a list of all available expectation functions?
